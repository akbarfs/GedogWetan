...
@endsection
          ...
          @section('main-content')
              @if ($JumlahDataPotensi>0)
              <div class="alert alert-success">Ditemukan {{ $JumlahDataPotensi}} data dengan kata kunci : {{$cari}}</div>
                  ...
                  @foreach($DataPotensi as $key => $Potensi)
                    <tr>
                      ...
                      <td align="center">
                        <form action="{{url('/potensi.'.$Potensi->id)}}" method="Post" onsubmit="return confirm('Apakah data ingin dihapus?')">
                        @csrf
                        <a href="{{url('/potensi.'.$Potensi->id.'.detail')}}" class="btn btn-xs btn-info" title="Detail"><i class="nc-icon nc-badge"></i></a>
                        <a href="{{url('/potensi.'.$Potensi->id.'.edit')}}" class="btn btn-xs btn-info" title="Edit"><i class="nc-icon nc-settings-gear-65"></i></a>
                  ...
                  <div class="alert alert-danger">Data dengan kata kunci : {{$cari}} Tidak Ditemukan
                      <a href="{{url('/potensi')}}"><input type="button" class="btn btn-danger" value="Klik Disini untuk kembali"></a>
                  </div>
                  @endif
