...
@endsection
          ...
          @if ($JumlahDataOrganisasi>0)
                  <div class="alert alert-success">Ditemukan {{ $JumlahDataOrganisasi}} data dengan kata kunci : {{$cari}}</div>
                  ...
                  @foreach($DataOrganisasi as $key => $Organisasi)
                    <tr>
                      ...
                      <td align="center">
                        <form action="{{url('/organisasi.'.$Organisasi->id_pengurus)}}" method="Post" onsubmit="return confirm('Apakah data ingin dihapus?')">
                        @csrf
                        <a href="{{url('/organisasi.'.$Organisasi->id_pengurus.'.edit')}}" class="btn btn-xs btn-info" title="Edit"><i class="nc-icon nc-settings-gear-65"></i></a>
                  ...
                  <div class="alert alert-danger">Data dengan kata kunci : {{$cari}} Tidak Ditemukan
                      <a href="{{url('/organisasi')}}"><input type="button" class="btn btn-danger" value="Klik Disini untuk kembali"></a>
                  </div>
                  @endif
