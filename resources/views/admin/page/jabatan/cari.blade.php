...
@section('content-header')
  <div class="content">
    ...
    <div class="col">
      <form method="get" action="{{url('/jabatan.cari')}}">
    ...
    <div class="col"><a href="{{url('/jabatan.tambah')}}">
...
@endsection
          ...
          @if ($JumlahDataJabatan>0)
                  <div class="alert alert-success">Ditemukan {{ $JumlahDataJabatan}} data dengan kata kunci : {{$cari}}</div>
                  ...
                  @foreach($DataJabatan as $key => $Jabatan)
                    <tr>
                      ...
                      <td align="center">
                        <form action="{{url('/jabatan.'.$Jabatan->id)}}" method="Post" onsubmit="return confirm('Apakah data ingin dihapus?')">
                        @csrf
                        <a href="{{url('/jabatan.'.$Jabatan->id.'.edit')}}" class="btn btn-xs btn-info" title="Edit"><i class="nc-icon nc-settings-gear-65"></i></a>
                  ...
                  <div class="alert alert-danger">Data dengan kata kunci : {{$cari}} Tidak Ditemukan
                      <a href="{{url('/jabatan')}}"><input type="button" class="btn btn-danger" value="Klik Disini untuk kembali"></a>
                  </div>
                  @endif
